...
async function searchFiles(fileType, apiKey, searchKey, threshold) {
    ...
    if (fileType == FILE_TYPES.TORRENTS) {
        ...
        results = files.map(result => toDownload(result, apiKey)); // Explicitly pass apiKey
    }
...
function toDownload(item, apiKey) {
    const downloadUrl = `${process.env.ADDON_URL}/resolve/RealDebrid/${apiKey}/${item.id}/${encode(item.download)}`;
    return {
        source: 'realdebrid',
        id: item.id,
        url: downloadUrl,
...
async function listFilesParrallel(fileType, apiKey, page = 1, pageSize = 50) {
    ...
    if (fileType == FILE_TYPES.TORRENTS) {
        return await RD.torrents.get(0, page, pageSize)
            .then(resp => {
                //console.log("Raw torrent API response:", resp);
                return resp.data || [];
            })
            .catch(err => handleError(err));
    } else if (fileType == FILE_TYPES.DOWNLOADS) {
        ...
        while (!finished) {
            const resp = await RD.downloads.get(0, currentPage, 50)
                .catch(err => handleError(err));
            //console.log("Raw downloads API response:", resp);
