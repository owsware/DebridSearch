...
function filterYear(torrent, cinemetaDetails) {
    // Check for IMDb ID match first
    if (torrent?.info?.imdb_id && cinemetaDetails?.imdb_id && torrent.info.imdb_id === cinemetaDetails.imdb_id) {
        console.log(`filterYear: IMDb ID match for ${torrent.name}, IMDb=${torrent.info.imdb_id}`);
        return true;
    }
